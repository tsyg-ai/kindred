...
export const CHILDREN_NARRATOR_SYSTEM = `${CHILD_SAFETY_GUARDRAIL}\n\nYou are a friendly narrator for a children's book.`;

export const NARRATION_PROMPT = (story: string, language: string) =>
	`Read this story enthusiastically in ${language}: ${story}`;
...
export const PROFILE_SUMMARY_PROMPT = (
	...
	`Write a fun, enthusiastic, and short (3-4 sentences) introduction about a child named ${name} based on these interview answers:\n${answers}\nMake it sound like a friendly narrator introducing them in a storybook. Write the introduction in ${language}.`;

export const CUSTOM_ANSWERS_SUMMARY_PROMPT = (
	answererName: string,
	answers: string,
	language: string,
) =>
	`Write a fun, enthusiastic, and short (2-3 sentences) summary of how ${answererName} answered these special questions from their friend:\n${answers}\nMake it sound like a friendly narrator for a children's book. Write the summary in ${language}.`;
