...
import { getEntryById, updateEntry, uploadBlobToStorage, FriendshipEntry } from '../services/db';
...
export default function StoryDetails() {
  ...
  const handleAnimateStory = async () => {
    ...
    try {
      ...
      setEntry(prev => prev ? { ...prev, videoProcessing: true } : null);
      ...
      const videoBlob = await generateStoryVideo(videoPrompt, entry.imageUrl);

      // Upload video to storage
      const videoUrl = await uploadBlobToStorage(videoBlob, `videos/entries/${id}.mp4`);

      // Save to Firestore; mark as newly ready (and no longer processing)
      await updateEntry(id, { videoUrl, videoProcessing: false, videoNewlyReady: false });
      setEntry(prev => prev ? { ...prev, videoUrl, videoProcessing: false, videoNewlyReady: false } : null);
    } catch (err) {
      console.error('Video generation failed:', err);
      await updateEntry(id, { videoProcessing: false }).catch(() => {});
      setEntry(prev => prev ? { ...prev, videoProcessing: false } : null);
    ...
    return (
      <div className="min-h-screen bg-wonderland flex items-center justify-center p-6">
        <div className="bg-white/80 rounded-[2rem] p-8 shadow-magic text-center max-w-sm">
          <p className="text-red-500 font-medium mb-4">{error || t('storyDetails.errors.load_failed')}</p>
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-magic-500 font-semibold hover:underline mx-auto">
  ...
  const canAnimate = !entry.videoUrl && !entry.videoProcessing && !isGeneratingVideo;
  ...
  return (
    <div className="min-h-screen bg-wonderland flex flex-col items-center p-6 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[8%] left-[5%] w-8 h-8 rounded-full bg-coral-300/30"
             style={{ animation: 'float 5s ease-in-out infinite' }} />
        <div className="absolute top-[30%] right-[6%] w-5 h-5 rounded-full bg-magic-300/25"
        ...
        <div className="absolute top-[60%] right-[12%] w-4 h-4 rounded-full bg-sunny-300/30"
      ...
      <div className="relative z-10 max-w-5xl w-full mt-8">
        ...
        >
          ...
          <div
            ...
            style={{ background: 'linear-gradient(160deg, #ede9fe 0%, #fce7f3 50%, #fef9c3 100%)' }}
          >
            <div className="w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-magic"
            ...
            <div className="flex flex-col gap-4 w-full">
              {entry.audioUrl && (
                <motion.button
                  ...
                  style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)' }}
                >
                  <Play size={24} fill="currentColor" /> {t('storyDetails.play_story')}
                </motion.button>
              )}
              ...
              {canAnimate && (
                ...
                >
                  <Film size={22} /> {t('storyDetails.animate_story')}
                </motion.button>
              )}
              ...
              {isProcessing && (
                <div className="w-full space-y-3">
                  <div className="bg-magic-50 border-2 border-magic-200 rounded-full py-4 px-6 flex items-center justify-center gap-3 text-magic-700 font-semibold">
                    ...
                    <span>{t('storyDetails.video_processing')}</span>
                  </div>
                  <p className="text-warm-500 text-sm text-center leading-relaxed px-2">
                    {t('storyDetails.video_generating_info')}
                  </p>
                </div>
              )}
